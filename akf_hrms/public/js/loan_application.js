frappe.ui.form.on("Loan Application", {
  loan_product: function (frm) {
    if (frm.doc.loan_product == "Advance Salary") {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            frm.set_value("maximum_allowed_loan", r.message.base / 2);
            frm.set_value("loan_amount", frm.doc.maximum_allowed_loan);
            frm.set_value(
              "repayment_amount",
              frm.doc.maximum_allowed_loan
            );
            frm.set_df_property("maximum_allowed_loan", "read_only", 1);
          } else {
      ...
      frm.set_df_property("maximum_allowed_loan", "read_only", 0);
    }
  },
  loan_amount: function (frm) {
    if (frm.doc.loan_product == "Advance Salary") {
      ...
      if (frm.doc.loan_amount > frm.doc.maximum_allowed_loan) {
        frappe.msgprint(
          __(
