...
# bench --site erp.alkhidmat.org execute akf_hrms.overrides.leave_application.leave_application.states
# def states():
#     c = 1
#     for d in frappe.db.get_list('Leave Application', filters={'docstatus': 1, 'workflow_state': 'Pending'}, fields=['*']):
#         print(f"{d.name} : {c}")
#         c = c + 1
#         data = frappe.db.sql(f""" 
# 			Select 
...
#         frappe.db.set_value('Leave Application', d.name, 'workflow_state', 'Applied')
#         doc = frappe.get_doc('Leave Application', d.name)
#         doc.custom_state_data = {}
#         doc.set_next_workflow_approver()
#         doc.record_application_state()
#         doc.save()
...
# 				if(frappe.db.exists('Has Role', {'parent': user_id, 'role': 'Line Manager'})):
# 					state_dict.update({
# 						"current_user": f"{self.workflow_state} (<b>{get_fullname(self.owner)}</b>)",
# 						"next_state": f"{self.custom_next_workflow_state} (<b>{self.leave_approver_name}</b>)" if(self.status=='Open') else "",
# 						"modified_on": get_datetime(),
# 					})
# 			else:
