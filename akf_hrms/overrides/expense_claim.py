...
@frappe.whitelist()
def get_travel_expense_amount(expense_date=None, travel_request=None, expense_type=None, custom_grade=None):
	if(not custom_grade):
		frappe.throw("Grade is not set. Please provide a valid grade to proceed with the validation.")
...
@frappe.whitelist()
def get_travel_attendance(expense_date=None, travel_request=None):
	
	duration = 0
	
	query_result= frappe.db.sql(f"""SELECT	
									att.custom_hours_worked
								FROM 
									`tabAttendance` att
								LEFT JOIN 
									`tabAttendance Request` ar ON ar.name = att.attendance_request
								WHERE 
									att.attendance_date = '{expense_date}' 
									AND ar.travel_request = '{travel_request}' """ ,as_dict = 1)
	
	if(query_result):
		dur = query_result[0].get('custom_hours_worked')
	else:
		frappe.throw(f"No attendance Record found against travel request '{travel_request}' for date '{expense_date}'")
