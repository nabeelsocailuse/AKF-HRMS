...
from frappe.utils import date_diff, add_days, getdate ,get_link_to_form, getdate,format_date, nowdate, time_diff
...
class OAttendanceRequest(Document):
	...
	def validate(self):
		...
		self.custom_from = str(f_hr).zfill(2) + ":" + str(f_min).zfill(2) + ":" + str(f_sec).zfill(2)
		self.custom_to = str(t_hr).zfill(2) + ":" + str(t_min).zfill(2) + ":" + str(t_sec).zfill(2)
	...
	def check_wfh_days(self):
		from_date=frappe.db.get_value("Work From Home Request", self.work_from_home_request, "from_date")
		to_date=frappe.db.get_value("Work From Home Request", self.work_from_home_request, "to_date")
		if from_date <= getdate(self.from_date) <= to_date:
			pass
		else:
			frappe.throw("Attendance Request date must be in between {0} and {1}".format(from_date, to_date))
	...
	def create_or_update_attendance(self, date: str):
		...
		if attendance_name:
			...
			frappe.msgprint(
					_("Updated In-Out Details from {0} to {1} for date {2} in the attendance record {3}").format(
