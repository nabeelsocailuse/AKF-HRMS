...
def record_workflow_approver_states(self, publish_progress=True):
	...
	approversList = frappe.parse_json(self.custom_tracking_information) if(self.custom_tracking_information) else []
	...
	frappe.db.set_value(self.doctype, self.name, 'custom_tracking_information', frappe.as_json(approversList))
	self.reload()
