...
def apply_policy(doc, method=None):
	...
	if(doc.shift): 
		grace = frappe.db.get_value('Shift Type', {"name": doc.shift}, ['custom_grace_in_time', 'custom_grace_out_time'], as_dict=1)
		# frappe.throw(f'grace: {grace}')
...
# bench --site erp.alkhidmat.org execute akf_hrms.utils.hr_policy.update_dle
def update_dle():
	# frappe.db.sql(''' delete from `tabDeduction Ledger Entry` where posting_date between '2025-07-01' and '2025-07-30' ''')
	# return
	for emp  in frappe.get_list('Employee', filters={'status': 'Active'}, fields=['name']):
		result = frappe.db.sql(f'''
			Select 
				employee, employee_name, company, shift, attendance_date, in_time, out_time, (4) as case_no
			From 
				`tabAttendance` d
			Where 
				status = 'Present'
				...
				and shift in (Select name from `tabShift Type` where name=d.shift and 
							((TIME_TO_SEC(TIMEDIFF(custom_grace_out_time, cast(d.out_time as time)))/3600)>0
							and (TIME_TO_SEC(TIMEDIFF(custom_grace_out_time, cast(d.out_time as time)))/3600)<=4))
		''', as_dict=1)
		# print(result)
		'''
		-- and employee = 'AKFP-PK-CO-00125'
		-- and shift in (Select name from `tabShift Type` where name=d.shift and cast(d.in_time as time)>custom_grace_in_time)
		'''
		"""
			1
			
			((TIME_TO_SEC(TIMEDIFF(cast(d.in_time as time), custom_grace_in_time))/3600)>0
							and (TIME_TO_SEC(TIMEDIFF(cast(d.in_time as time), custom_grace_in_time))/3600)<=2))
			2
			
			((TIME_TO_SEC(TIMEDIFF(cast(d.in_time as time), custom_grace_in_time))/3600)>2
							and (TIME_TO_SEC(TIMEDIFF(cast(d.in_time as time), custom_grace_in_time))/3600)<=4))
				
			3
			
			(TIME_TO_SEC(TIMEDIFF(cast(d.in_time as time), custom_grace_in_time))/3600)>4)
		...
		for d  in result:
			...
			if(d.case_no ==2 ):
				...
				args = frappe._dict({
					'company': d.company,
					'employee': d.employee,
					'posting_date': d.attendance_date,
					"case_no": d.case_no,
					"doctype": "Deduction Ledger Entry"
				})
				verify_case_no(args)
			elif(d.case_no ==3 ):
				print("Yes")
				...
				if(count == 3):
					...
					print("No")
		# if(d.shift):
		# 	grace = frappe.db.get_value('Shift Type', {"name": d.shift}, ['custom_grace_in_time', 'custom_grace_out_time'], as_dict=1)
		# 	# frappe.throw(f'grace: {grace}')
		# 	start_time = grace.custom_grace_in_time if(grace.custom_grace_in_time) else grace.start_time
		# 	end_time = grace.custom_grace_out_time if(grace.custom_grace_out_time) else grace.end_time
	# apply_policy(doc, method=None)
