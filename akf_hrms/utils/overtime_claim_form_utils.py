...
def record_workflow_approver_states(self, publish_progress=True):
	...
	approversList = frappe.parse_json(self.tracking_information) if(self.tracking_information) else []
	...
	for d in get_next_role_employee(wf.allowed, self.department):
		...
		frappe.db.set_value(self.doctype, self.name, 'next_workflow_approver', d.name)
	...
	frappe.db.set_value(self.doctype, self.name, 'tracking_information', frappe.as_json(approversList))
	self.reload()
