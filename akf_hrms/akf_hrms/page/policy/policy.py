...
def get_policy(fargs):
	...
	if 'HR Manager' in frappe.get_roles(frappe.session.user):
		...
		if emp_company:
			get_policy_ = frappe.db.sql("""
						Select 
							epm.name, IFNULL(epm.policy_contents,""), IFNULL(policy_file, "/desk#Form/Policy/Policy") 
						From 
							`tabEmployee Policy Management` as epm, `tabFiscal Year Company` as fyc 
						Where 
							epm.status ='Active'  
							and fyc.parent=epm.name 
							and fyc.company='{0}'
							{1}
						Order by 
							sort_order""".format(emp_company[0][0], get_conditions(fargs)), fargs)
...
def get_conditions(fargs):
	...
	conditions = " and custom_department=%(department)s" if(department and department!="") else ""
	return conditions