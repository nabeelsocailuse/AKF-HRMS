...
from akf_hrms.akf_hrms.doctype.expense_claim.one_day_salary import get_one_day_salary # Mubarrim, April 18-2025
...
# Mubashir Bashir 15-01-2025 End	
	...
	def get_travel_attendance(self):
		...
		for expense in self.expenses:
			if expense.expense_type == "Daily Allowance":
				query_result = frappe.db.sql(f"""
					SELECT att.custom_hours_worked
				...
				if dur in ['0', '0.0', '', 'None', None]:
					...
					try:
						...
						try:
							...
							frappe.throw(f"Invalid time format: '{dur}'")
