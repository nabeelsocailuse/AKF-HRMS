...
class WorkFromHomeRequest(Document):
	def validate(self):
		...
		if self.employee:
			exceed_wfh_limit = frappe.db.get_value("Employee", self.employee, "exceed_wfh_limit")
	...
	def check_limit_per_month(self):
		...
		total_ = 0.0
		get_previos_data = frappe.db.sql(""" select ifnull(sum(total_days),0) from `tabWork From Home Request` where docstatus=1 
		...
		if total_days_ > float(get_total_days):
			frappe.throw("Your monthly limit exceed for {0} days WFH, please coordinate with HR department".format(get_total_days))
			
	def get_holiday(self):
		holidays = 0
		if self.holiday_list:
			get_count = frappe.db.sql_list('''select day(holiday_date) from `tabHoliday` where parent=%s and holiday_date between %s and %s ''', (self.holiday_list, self.from_date, self.to_date))
			if get_count:
...
def chk_dublication(employee,doc_name):
	name_ = ''
	wfh_name = frappe.db.sql(""" select name from `tabAttendance Request` where employee=%s and work_from_home_request=%s and docstatus!=2 """,(employee,doc_name))
	if wfh_name:
		name_ = wfh_name[0][0]
	return name_
