...
def get_vacancies_from_staffing_plan(company, branch, department, designation, posting_date):
    staffing_plan = frappe.db.sql("""
        SELECT sp.name, spd.vacancies, spd.workstation_availability,
			spd.qualification, spd.laptop_required, spd.position_type
    ...
    if staffing_plan:
        ...
        workstation_availability = staffing_plan[0].workstation_availability
        qualification = staffing_plan[0].qualification
        laptop_required = staffing_plan[0].laptop_required
        position_type = staffing_plan[0].position_type

        current_count = frappe.db.count("Employee", 
            filters={"status": "Active", "designation": designation, "branch": branch, "department": department}
        )
        ...
        frappe.msgprint(f'Current Count: {current_count}')

        return {
            "staffing_plan": staffing_plan_name,
            "remaining_vacancies": remaining_vacancies,
            "current_count": current_count,
            "workstation_availability": workstation_availability,
