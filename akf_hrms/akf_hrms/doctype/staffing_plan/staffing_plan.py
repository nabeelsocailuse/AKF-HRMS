...
class StaffingPlan(Document):
	...
	def set_total_estimated_budget(self):
		...
		for detail in self.get("staffing_details"):
			...
			designation_counts = get_designation_counts(self.department, detail.designation, self.company)
...
@frappe.whitelist()
def get_designation_counts(department, designation, company, job_opening=None):
	if not designation:
		return False
	...
	employee_count = frappe.db.count(
		"Employee", {"designation": designation, "department": department, "status": "Active", "company": ("in", company_set)}
	)
...
def get_active_staffing_plan_details(company, designation, from_date=None, to_date=None):
	...
	staffing_plan = frappe.db.sql(
		"""
		select sp.name, spd.vacancies, spd.total_estimated_cost, spd.current_count, spd.number_of_positions
		from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
