...
class Gratuity(AccountsController):
	...
	def on_cancel(self):
		...
		if (not self.pay_via_salary_slip): self.create_gl_entries(cancel=True) # Nabeel Saleem, 14-04-2025
...
def get_total_applicable_component_amount(
	...
	# frappe.throw(f"{employee}, {applicable_earnings_component}, {gratuity_rule}")
