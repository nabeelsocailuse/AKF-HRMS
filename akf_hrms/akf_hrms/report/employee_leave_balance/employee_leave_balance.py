...
def get_data(filters: Filters) -> list:
	...
	for leave_type in leave_types:
		...
		for employee in active_employees:
			...
			# nabeel saleem
			_expired_leaves_ = flt(expired_leaves, precision)
			_leaves_taken_ = flt(leaves_taken, precision)

			if(leave_type in ['Half Leave', 'Half Day Leave', 'Short Leave']):
				_leaves_taken_ =  flt(leaves_taken+leaves_taken, precision)
				_expired_leaves_ = flt((expired_leaves-expired_leaves), precision)
				
			row.leaves_expired = flt(_expired_leaves_, precision)
