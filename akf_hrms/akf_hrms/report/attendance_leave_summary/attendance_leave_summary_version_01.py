...
def get_employee_data(filters):
	...
	emp_record = """
		SELECT 
			e.name, e.employee_name, e.branch, e.department, e.designation, e.employment_type, e.custom_no_attendance,
			COALESCE(e.holiday_list, '') as _holiday_list
		FROM `tabEmployee` as e
		WHERE 
			e.status = 'Active'
			{condition} 
			-- AND e.custom_no_attendance != 1 
