...
def get_absentees(filters, user):
	...
	if "HR Manager" in frappe.get_roles(user):
		# Query string
		absent_query = """ SELECT employee, employee_name, custom_gender, custom_designation, department, custom_region, status FROM `tabAttendance` att
		...
		absent_query = """SELECT att.employee, att.employee_name, custom_gender, att.custom_designation, att.department, att.custom_region, att.status FROM `tabAttendance` att, `tabUser Permission` per 
...
def get_check_in_out(filters, user):
	...
	if "HR Manager" in frappe.get_roles(user):
		...
		query=""" SELECT  att.employee, att.employee_name, att.custom_designation, att.department, att.custom_region, att.attendance_date,
            ...
            WHERE docstatus=1 and status='Present' and (in_time is null or out_time is null)
...
def get_pending_leaves(filters, user):
	...
	if "HR Manager" in frappe.get_roles(user):
		query =""" 
                SELECT employee, employee_name, leave_type, department, custom_designation, from_date, to_date, total_leave_days, status 
...
def get_approved_leaves(filters, user):
	...
	if "HR Manager" in frappe.get_roles(user):
		query =""" 
				SELECT employee, employee_name, department, custom_designation, from_date, to_date, total_leave_days, leave_type
		...
		query =""" 
                SELECT la.employee, la.employee_name, la.department, la.designation, la.from_date,la.to_date, la.total_leave_days, la.leave_type
                FROM `tabLeave Application` la, `tabUser Permission` per
